...
while [ "$1" != "" ] ; do
  case $1 in
    "-h" | "-H" | "-?" | "?")
       ...
       echo "  state using '-c'"
       echo
       echo "Syntax: $0 [-c|-i|-?|-h|-v] | [-R|-RA|-A|-AP][-I] [--no-interactive <Y|N>] [--no-verbose] [--version]"
       echo
       echo "  Parameters:"
       ...
       echo "        -CP Allows to use a custom /24 prefix to be used."
       echo ""
    ...
    "-CP")
       ...
       ERR_MSG=""
       if [ "$1" == "" ] ; then
         ERR_MSG="-CP requires an IPv4 IP prefix (only the IP portion). Try -? for more info"
       else
         _ipNum=( $(echo "$1" | sed -e "s#[\.\/]# #g") )
         case ${#_ipNum[@]} in
           5)
             ERR_MSG="-CP:'$1' Only provide the IP portion of the prefix!"
             ;;    
           1 | 2 | 3 | 4)
             regex='^[0-9]+$'
             for _num in ${_ipNum[@]} ; do
               if [[ $_num =~ $regex ]] ; then
                 if [ $_num -lt 0 ] || [ $_num -gt 255 ] ; then
                   ERR_MSG="Value '$_num' must be bewteen 0..255"
                 fi
               else
                 ERR_MSG="'$_num' is not a valid number!"
                 break;
               fi
             done
             ;;
           *) 
             ERR_MSG="-CP: '$1' is not a valid prefix!"
             ;;
         esac
       fi
       if [ "$ERR_MSG" != "" ] ; then
       ...
       _CUSTOM_PFX_24="${_ipNum[0]}.${_ipNum[1]}.${_ipNum[2]}.${_ipNum[3]}"
       verifyIPv4 $_CUSTOM_PFX_24
       if [ $? -eq 1 ] ; then
         println "Parameter -AP is an invalid IP prefix $_CUSTOM_PFX_24/24!"
         exitPrg 1
       fi
       ;;
    "--no-interactive")
...
println "NIST BRIO EXAMPLE configurator."
...
if [ $_P_ROLLBACK -eq 1 ] ; then
  ...
  for _folder in "${_FOLDERS[@]}" ; do
    # This array contains "from" "to" tupples
    search=( ".tpl"           ".$_CONF_EXT" 
             "README.tpl.md" "README.md" 
             ".tpl.md"        "-$_CONF_EXT.md" )
    idx=0
    while [ $idx -lt ${#search[@]} ] ; do
      _fromExt=${search[$idx]}; ((idx++))
      _toExt=${search[$idx]};   ((idx++))

      ls $_folder/*$_fromExt > /dev/null 2>&1
      if [ $? -eq 0 ] ; then
        println "Process '$_folder'..." 
        _tpl_files=( $(ls $_folder/*$_fromExt | sed -e "s/\(.*\)$_fromExt/\1$_toExt/g" | sed -e "s#//#/#g") )
...
if [ $_P_CHECK_ONLY -eq 0 ] ; then
  ...
  for folder in "${_FOLDERS[@]}" ; do
    cd $folder
    # Determine if templates exist .tpl or .tpl.md
    _TPL_FILES=( $(ls | grep -E '\.((README\.)?tpl.md|tpl)$' ) )
    if [ ${#_TPL_FILES[@]} -gt 0 ] ; then
      ...
      for template in "${_TPL_FILES[@]}" ; do
        ...
        cp "$template" "$template-$_TPL_CURR"
        if [ ! -e $template-$_TPL_CURR ] ; then
        ...
        for as_config in "${_IP_CONFIG[@]}" ; do
          ...
          cat "$template-$_TPL_CURR" \
              | sed -e "s/{IP_AS_$_ASN}/$_ASIP/g" \
              | sed -e "s/\.tpl)/\.conf)/g" \
              | sed -e "s/\.README.tpl.md)/\.README.md)/g" \
              | sed -e "s/(README.tpl.md)/(README.md)/g" \
              | sed -e "s/\.tpl.md)/\-conf.md)/g" \
              > "$template-$_TPL_NEXT"
          rm "$template-$_TPL_CURR"
          _TPL_CURR=$_TPL_NEXT
        ...
        # The first sed changes .tpl to .conf or .tpl.md to .conf.md, 
        # the second sed changes .conf.md to -conf.md, 
        # the thirs sed changes README-conf to README
        new_template=$(echo $template \
                       | sed -e "s/\.tpl/.$_CONF_EXT/g" \
                       | sed -e "s/\.$_CONF_EXT.md$/-$_CONF_EXT.md/g" \
                       | sed -e "s/README-$_CONF_EXT/README/g")
        mv "$template-$_TPL_CURR" "$new_template"
        if [ -e "$new_template" ] ; then
          println "]...OK"
        else
          println "ERROR: Could not generate template '$new_template'"
        fi
      done
    fi
