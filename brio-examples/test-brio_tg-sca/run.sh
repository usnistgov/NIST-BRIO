...
DEMO_KEYS_FLDR=$DEMO_PREFIX_FLDR/opt/brio-examples/bgpsec-keys
...
{
  echo
  echo "Syntax: $0 <module> [--report] [--keep-datafile]"
  echo
  ...
  echo "                    in brio_tg-sca-2 and brio_tg-sca-3"
  echo
  echo "Modules:"
  echo "  brio-sca-1 Test srxcryptoapi using bgpsecio by dynamically"
  echo "             generating signatures and have srxcryptoapi validate them"
  echo "  brio-sca-2 Generate binary data file containing BGPsec_PATH attribute using"
...
CFG_FILES=("$DEMO_FLDR/srxcryptoapi.conf"
           "$DEMO_FLDR/brio_tg-sca-replay.conf"
           "$DEMO_FLDR/brio_tg-sca-gen.conf"
...
case "$_module" in
  "brio-sca-1")
      _CONF_FILE_1="$DEMO_FLDR/brio_tg-sca-gen.conf"
      _DATA_FILE=""
      ;;
  "brio-sca-2")
      _CONF_FILE_1="$DEMO_FLDR/brio_tg-sca-gen.conf"
      _CONF_FILE_2="$DEMO_FLDR/brio_tg-sca-replay.conf"
...
if [ $retVal -eq 0 ] ; then
  if [ $__REPORT -eq 0 ] ; then
    if [ "$_DATA_FILE" == "" ] ; then
      # run brio+tg-sca-1 without report
      startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_1 -m CAPI 2>/dev/null
      retVal=$?
      echo "It is expected to have 1 invalid and 3 valid routes!"
    else
      # run brio_tg-sca-2 or brio_tg-sca-3 without report
      startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_1 -m GEN-C --out $_DATA_FILE 2>/dev/null
      retVal=$?
      if [ $retVal -eq 0 ] ; then
        startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_2 -m CAPI --bin $_DATA_FILE 2>/dev/null
        retVal=$?
      fi
    fi
  else
    if [ "$_DATA_FILE" == "" ] ; then
      # run brio_tg-sca-1 with report
      _RESULT=$(startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_1 -m CAPI 2>/dev/null \
                | grep -e "$_ERROR_REGEX" | wc -l)
    else
      # run brio_tg-sca-2 with report
      startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_1 -m GEN-C --out $_DATA_FILE >/dev/null 2>&1
      retVal=$?
      if [ $retVal -eq 0 ] ; then
        _RESULT=$(startPrg 0 "./$PRG_BRIO" "-f" $_CONF_FILE_2 -m CAPI --bin $_DATA_FILE 2>/dev/null \
                  | grep -e "$_ERROR_REGEX" | wc -l)
      else
        _RESULT=0
      fi
