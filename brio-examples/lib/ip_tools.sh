...
#
#
IP_TOOLS_LIB_VER=b0.7.0.1

# Alias can be empty. If not then it is used as prefix for alias interfaces.
# E.g. interface ens18 with ALIAS="brio" will result in alias interfaces names
# ens18:brio0, ens18:brio1, and so on.  
ALIAS="brio"
  ...
  if [ "$ipAddr" != "" ] && [ "$iFace" != "" ] ; then
    ...
    if [ $? -eq 0 ] ; then  
      ...
      if [ $retVal -eq 0 ] ; then
        ...
        aliasNumArr+=( $(ifconfig | grep -e "^$iFace:$ALIAS[^: ]\+: .*" | awk '{ print $1 }' \
                                  | sed -e "s/.*:$ALIAS\([0-9]\+\):$/\1/g" | sort -u -g ) )
        ...
        print "Add $ipAddr to $iFace:$ALIAS$nextNum..."
        sudo ifconfig $iFace:$ALIAS$nextNum $ipAddr > /dev/null 2>&1
        retVal=$?
        if [ $retVal -eq 0 ] ; then
          println "done!"
          _IPT_ADDIP_IFACE_ALIAS+=( $iFace:$ALIAS$nextNum:$ipAddr )
        else
          println "failed!"
