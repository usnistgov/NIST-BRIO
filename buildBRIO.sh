#!/bin/bash
# This script is auto-generated by BGP-SRx Software Suite Bundler V9.4.7

# Date: Wed Aug  6 22:34:54 EDT 2025

SCRIPTNAME='BRIO Software Suite Vb7.1.2 Build script V9.4.7!'
...
function exit_script()
{
  ...
  echo $SCRIPTNAME
  echo "2017/2025 by Oliver Borchert (NIST)"
  echo
  exit $EXIT_CODE
}

# This function verifies that the script is running on the intended platform.
function checkOS()
{
  case "$OS_NAME" in 
  "rocky" |"ubuntu")
      ;;
    *)
      echo "  $OS_NAME is not supported by the installation framework!"
      echo "  Please see the individual installation requirements in each"
      echo "  component folder to build the framework manually!"
      echo
      echo "  The build script was generated for the following list of"
      echo "  systems: Rocky 9, Ubuntu 22.04"
      echo
      exit_script 1
      ;;
  esac
...
LOC_PREFIX=$(echo $INSTALL_DIR/local | sed -e "s/\/\.\//\//g")

# This is used for the .vscode json files.
REL_LOC_PREFIX="local"
...
function createVsCode()
{
  ...
  local ROOT_BRIO=$( ls | grep bgprpki-io )
...
##   The hash "#" symbol will be replaced with space " " later on.
  ...
  if [ $DO_BRIO -eq 1 ] ; then
    packages+=("BRIO")
    debugger+=("BRIO_TG#$ROOT_BRIO#bin/brio_tg#-f#../etc/brio-demo.cfg")
    debugger+=("BRIO_RC#$ROOT_SRxSnP/src#bin/brio_rc#50000#-f#../etc/cache.conf")
  fi
...
function syntax_module()
{
  ...
  while [ "$1" != "" ] ; do
    case $1 in
      ...
      "EXAMPLES") syntax_module_banner $1; $0 $1 -X examples 1 -? -C -A ;;
...
function syntax()
{
  ...
  echo "    -X <sca|brio|examples> <num> <parameter>*"
...
if [ $DO_BRIO -eq 1 ] ; then
  ...
  if [ $DO_CONFIGURE -eq 1 ] ; then
    # Check if a configure script exist, otherwise try to generate one!
    checkConfig
    ./configure --prefix=$LOC_PREFIX sca_dir=$LOC_PREFIX CFLAGS="-O0 -g" ${PARAM_BRIO[@]}
    check $? "Error Configuring $NAME!"
  fi
  make_install $NAME
fi
